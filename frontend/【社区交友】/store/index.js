 ...
 * 		  (1) 渲染到页面
 * 		  (2) 存储到本地存储
 * 			  a. chatdetail_17_12   *这种方式需要加密 xubowen注*
 * 			  b. chatlist17（将当前会话置顶，修改chatlist中当前会话的data和time显示） *17用户的会话列表信息*
 * 
 * 场景二：与当前用户12不处于聊天界面
 ...
 * 接收到的消息格式：
{
	...
	from_userpic:"http://pic136.nipic.com/file/20170725/10673188_152559977000_2.jpg",  *为啥是jpg？*
	type:"text",	 // 发送类型
	data:"你好啊",	 // 发送内容
	time:151235451   // 接收到的时间  *unix时间戳*
}
 * 
 * 发送消息
 * 		(1) 写入存储
 * 			chatdetail_17_12
 * 			chatlist17（将当前会话置顶，修改chatlist中当前会话的data和time显示） *更新这个*
 * 		(2) 请求ajax发送消息 
 * 		(3) 渲染到页面（user-chat页面实现）  *这其实应该等server ack之后*
 * 
 * 读取消息
 * 		(1) 写入存储
...
import $Chttp from '@/common/request-chat.js'
...
export default new Vuex.Store({
	...
	mutations:{
		...
		login(state,user){
			...
			state.user = user
			state.token = state.user.token  // * ...... xubowen注*
		...
		// 修改用户信息(手机号,邮箱,密码)  *这个 interface 封装的不好*
		editUserInfo(state,{ key,value }){
			state.user[key] = value
			uni.setStorageSync('user', JSON.stringify(state.user));
		},
		// 修改资料  // *obj 应该叫 user_info_new*
	...
	actions:{
		...
		openSocket({ state,dispatch }){
			...
			state.SocketTask.onMessage((e)=>{
				...
				// *也就是说只能有bind 和 text 两种类型的msg*
				// 绑定返回结果
				
				if (res.type == 'bind'){
					// 用户绑定
					// *ws 里来了bind 就去http post一个bind？*
		...
		userBind({state,dispatch},client_id){
			console.log("userBind post")
			$Chttp.post('/chat/bind',{
				type:"bind",
				client_id:client_id,
				id: state.user.id
			},{
		...
		// 获取未读信息
		async getUnreadMessages({state,dispatch}){
			console.log('获取未读信息中...');
			let data = await $Chttp.post('/chat/get',{id: state.user.id},{
				token: true,
			})
			console.log('获取未读信息成功',data);
			for (const msg of data) {
				// 处理接收消息
				await dispatch('handleChatMessage',msg)
			}
		...
		// 处理接收消息
		async handleChatMessage({state,dispatch},data){
			console.log('处理接收消息',data);
			// 全局通知接口
			uni.$emit('UserChat',data);
			// 存储到chatdetail
			await dispatch('updateChatDetailToUser',{
				data,
				send:false
			})
			let list = await dispatch('getChatDetailToUser',data.from_id)
			console.log(list)
			// 更新会话列表
		...
		async updateChatDetailToUser({state,dispatch,commit},e){
			 ...
			 await list.push(await dispatch('formatChatDetailObject',e))
			 // 存储到本地存储
			 await commit('saveChatDetail',{
			 	list,toId
			 })
		},
		// 发送消息
