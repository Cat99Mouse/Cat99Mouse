		...
		<swiper :current="tabIndex" :duration="150" :style="'height:'+scrollH+'px;'" @change="onChangeTab">
			...
			<swiper-item>
				<scroll-view scroll-y="true" :style="'height:'+scrollH+'px;'">
					...
					<divider></divider>
					<!-- 最近更新 -->
					<view class="p-2 font-md">Recent</view>
					<!-- 话题列表组件 -->
		...
		methods: {
			...
			getList(){
				...
				this.$H.get('/follows/'+this.page,{},{
					...
					this.loadmore  = list.length < 10 ? 'Nothing to show' : 'Pull up to load more';
				}).catch(err=>{
					if(!isrefresh){
						this.page--;
					}
			...
			doSupport(e){
				...
				uni.showToast({ title: msg + 'succeed' });
			},
			// 上拉加载
			loadmoreEvent(){
				// 验证当前是否处于可加载状态
				if (this.loadmore !== 'Pull up to load more') return;
				// 设置加载状态
