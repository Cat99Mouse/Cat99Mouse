	...
	<view>
		...
		<template v-if="tabIndex === 0">
			<view class="animated fast fadeIn">
				<view class="p-3 border-bottom">
					<view class="font-md">Account info</view>
					<view class="font">Age：{{userinfo.regtime}}</view>
					<view class="font">Count id：{{user_id}}</view>
				</view>
				<view class="p-3 border-bottom">
					<view class="font-md">Personal details</view>
					<view class="font">Constellation：{{userinfo.birthday}}</view>
					<view class="font">Job：{{userinfo.job}}</view>
					<view class="font">Hometown：{{userinfo.path}}</view>
					<view class="font">Emotion state：{{userinfo.qg}}</view>
				</view>
			</view>
		</template>
		<template v-else>
		...
		<uni-popup ref="popup" type="top">
			...
			<view v-if="!userinfo.isblack" class="flex align-center justify-center font-md py-2" hover-class="bg-light" @click="openChat">
				<text class="iconfont icon-shanchu mr-2"></text> Chat
			</view>
		</uni-popup>
